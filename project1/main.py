...
correct = 0
for i in range(test_bow_features.shape[0]):
    if(test_labels[i]*(np.dot(theta, test_bow_features[i]) + theta_0) > 0.001):
        correct = correct + 1
accuracy_test = correct / test_bow_features.shape[0]
print("Question 8: Accuracy = {}".format(accuracy_test))

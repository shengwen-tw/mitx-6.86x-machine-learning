...
def project_onto_PC(X, pcs, n_components, feature_means):
    ...
    for i in range(0, X.shape[0]):
        for j in range(0, X.shape[1]):
            X_centered[i, j] = X[i, j] - feature_means[j]

    # Calculate X_tilde * V
    # return np.dot(X_centered, pcs)[:, :n_components]

    # Multiply -1 on local machine if direction is wrong
    # (Since eigen vectors have no canonical direction)
    return -1 * np.dot(X_centered, pcs)[:, :n_components]
